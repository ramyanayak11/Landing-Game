...
void Octree::subdivide(const ofMesh &mesh, TreeNode &node, int numLevels, int level) {
	...
	vector<Box> childBoxList;
	subDivideBox8(node.box, childBoxList);						// subdivide current box into 8

	for (int i = 0; i < childBoxList.size(); ++i) {				// for each child / sub-box
		TreeNode childNode;
		childNode.box = childBoxList[i];

		vector<int> childPoints;
		getMeshPointsInBox(mesh, node.points, childNode.box, childPoints);

		if (childPoints.size() > 0) {							// if there's at least 1 point in that child's box
			childNode.points = childPoints;
			node.children.push_back(childNode);					// add that child as a child of curr node

			if (childPoints.size() > 1) {
				subdivide(mesh, node.children.back(), numLevels, level+1);	// if there's >1 point, recurse & subdivide further
			}
		}
	}
...
// similar to the ray-box intersection done in lab 2
bool Octree::intersect(const Ray &ray, const TreeNode &node, TreeNode &nodeRtn) {
	if (!node.box.intersect(ray, 0.001, 10000.0)) {				// base case: if no intersection, return false
		return false;
	}

	bool intersects = false;

	if (node.children.size() == 0) {							// if leaf node, return intersects = true
		nodeRtn = node;
		intersects = true;
	}
	else {														// otherwise, recursively check for intersection
		for (int i = 0; i < node.children.size(); ++i) {		// with its children
...
bool Octree::intersect(const Box &box, TreeNode &node, vector<Box> &boxListRtn, vector<Box> &leavesBoxListRtn) {
	if (box.overlap(node.box)) {								// if current node's box intersects,
		boxListRtn.push_back(node.box);							// add to returning box list

		if (node.children.size() == 0) {
			leavesBoxListRtn.push_back(node.box);
		}

		for (int i = 0; i < node.children.size(); ++i) {
			intersect(box, node.children[i], boxListRtn, leavesBoxListRtn);		// recursively check if children's boxes intersect
		}
	}
...
void Octree::draw(TreeNode &node, int numLevels, int level) {
	...
	for (int i = 0; i < node.children.size(); ++i) {
		draw(node.children[i], numLevels, level+1);		// recursively draw children
	}
